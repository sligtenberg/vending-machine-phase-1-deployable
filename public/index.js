...
const populateSnackDisplay = () => {
    fetch(`${window.location.origin}/snacks`)
    .then(res => res.json())
...
const populateCashDrawer = () => {
    fetch(`${window.location.origin}/cashes`)
    .then(res => res.json())
    .then(cashDrawer => {
...
const updateSnackOnServer = snack => {
    fetch(`${window.location.origin}/snacks/${snack.id}`, {
        method: 'PATCH',
...
const sendMoneyInCashDrawer = moneyToSend => {
  moneyToSend.forEach(denomination => {
    fetch(`${window.location.origin}/cashes/${denomination.id}`, {
      method: 'PATCH',
      headers: {'Content-Type': 'application/json'},
